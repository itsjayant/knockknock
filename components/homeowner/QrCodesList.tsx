...
export default function QrCodesList({ initialQrCodes, onNewQrCode }: Props) {
  ...
  function handleDownloadQR(qrId: string, label: string) {
    ...
    // Create a canvas to draw the QR code with label and branding
    ...
    img.onload = () => {
      ...
      const topText = "Scan to Ring Bell";
      const branding = "🔔 KnockKnock";
      const fontSize = 18;
      const brandFontSize = 14;

      // Set canvas dimensions
      canvas.width = imgSize + padding * 2;
      canvas.height = imgSize + padding * 2 + 140; // Extra space for text
      ...
      ctx.fillText(topText, canvas.width / 2, padding - 5);
      ...
      ctx.textAlign = "center";
      ctx.fillText(label, canvas.width / 2, imgSize + padding + spacing + 25);

      // Draw branding at bottom
      ctx.fillStyle = "#71717a";
      ctx.font = `${brandFontSize}px Arial`;
      ctx.textAlign = "center";
      ctx.fillText(branding, canvas.width / 2, imgSize + padding + spacing + 60);
